...
- [SGLang 架构与调度循环源码走读](学习整理/SGlang.md) — SGLang 三进程模型（Tokenizer/Scheduler/Detokenizer）+ 三层架构走读，逐步拆解 Scheduler 事件循环 recv→分发→组批→run→result：含组批优先级策略（FCFS/LPM/DFS-Weights/LOF…）、采样算法、grammar 约束、eager/CUDA Graph、RadixCache 前缀复用与注意力 backend 选型；新增 Detokenizer 增量解码（BPE 边界 / UTF-8 半字符 / trim_matched_stop）与 SSE 流式输出协议。Updated: 2026-08-13
