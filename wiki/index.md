...
- [SGLang 架构与调度循环源码走读](学习整理/SGlang.md) — SGLang 三进程模型（Tokenizer/Scheduler/Detokenizer）+ 三层架构走读，逐步拆解 Scheduler 事件循环 recv→分发→组批→run→result：含组批优先级策略（FCFS/LPM/DFS-Weights/LOF…）、采样算法、grammar 约束、eager/CUDA Graph 与注意力 backend 选型；**RadixCache 与 KV 内存池深挖**——树的增删改查、7 种驱逐策略统一为 `get_priority` 排序键（含级联向上驱逐）、`lock_ref` 如何把前缀移出可驱逐账本、前缀匹配的指数搜索+二分、Token-to-page 两级映射（`req_to_token` 表的 padding 行由来、三个 alloc 入口分工）；含国产 GPU/NPU 适配文件清单（昇腾树内全栈 / 摩尔线程 MUSA / 昆仑芯等走树外插件）与 Detokenizer 增量解码（BPE 边界 / UTF-8 半字符 / trim_matched_stop）+ SSE 流式输出协议。Updated: 2026-08-13
