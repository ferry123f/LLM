...
单卡装不下、或者装得下但算不快时，就要把模型和数据摊到多张卡上。这篇讲**四种并行策略**（DP/PP/TP/EP）、它们**共用的通信原语**，以及为什么在推理场景里各自的地位和训练完全不同。
...
decode 是 **memory-bound** 的，耗时约等于「把权重从 HBM 搬一遍」的时间。TP=4 之后每张卡**只需搬 1/4 的权重**，而 4 张卡的带宽是并行叠加的——**理论上 decode 能快接近 4 倍**。这是 TP 在推理里压过 PP 的根本原因：**PP 切层不减少单卡要搬的权重总量的占比时间**，TP 切张量则实打实地把带宽压力分摊了。
...
- [[SGlang]] — 本篇 §五 的展开：`distributed/` 目录、PD 分离、以及通信算子如何被注册成 custom op。
...
- 本文的并行策略与通信原语部分整理自学习笔记与配图；**图为示意，非某一框架的实际实现**。
- §五 的 SGLang 参数与文件路径核对自 `d:/project/sglang`，commit `fdebc938f7`（tag `v0.5.16`）；**参数名可能随版本变化，以 `--help` 为准**。
